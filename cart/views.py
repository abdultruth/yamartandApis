...
def add_cart(request, product_id):
    ...
    if current_user.is_authenticated:
        ...
        # Do need get_cart_id because we are in authenticated session or already logged in
        ...
        if is_cart_item_exits:
           ...
           if product_variation in ex_var_list: 
                #Increase cart_item Quantity if the product_variation already exist in cart
            ...
            # if there is no product_variation existing in cart, create 
                item = CartItem.objects.create(product=product, quantity=1, user=current_user)
            
                if len(product_variation) > 0:
                  item.variations.clear()
                  item.variations.add(*product_variation[1])
                  item.variations.add(*product_variation[2])
                item.save()
            ...
            if len(product_variation) > 0:
                    ...
                    cart_item.variations.add(*product_variation[1])
                    cart_item.variations.add(*product_variation[2])
            cart_item.save()
    ...
    # if the user is not authenticated
        ...
        try:
            cart = Cart.objects.get(cart_id=_get_cart_id(request)) #get the cart_id present in the session 
...
def remove_cart(request, product_id, cart_item_id):
    try:
       ...
       if request.user.is_authenticated:
          cart_item = CartItem.objects.get(product=product, user=request.user, cart_item_id=cart_item_id )
       else:   
